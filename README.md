# Аналитика роста и удержания TheLook
...
**Сквозной портфельный проект продуктового аналитика:** от сырых данных и контроля качества до продуктовых метрик, когортного удержания, RFM, статистических проверок, A/B-симуляции и рекомендаций бизнесу.
...
[Открыть выполненный ноутбук Pandas](notebooks/01_thelook_pandas_analysis.ipynb) · [Посмотреть SQL для BigQuery](sql/README.md) · [Прочитать разбор проекта](docs/CASE_STUDY.md) · [Подготовиться к защите](docs/INTERVIEW_GUIDE.md)
...
|---|---:|---|
| Чистая выручка, прокси | **$8,07 млн** | Выручка после исключения отмен и возвратов |
| Валовая прибыль, прокси | **$4,19 млн** | Выручка за вычетом себестоимости товара |
| Валовая маржа | **51,9%** | Маржа до маркетинга, логистики и налогов |
| Доля возвратов | **11,8%** | Существенная зона потери выручки |
| Доля отмен | **15,0%** | Каждый шестой-седьмой заказ отменяется |
| Доля повторных покупателей | **30,4%** | Покупатели с двумя и более заказами без отмен и возвратов |
| Удержание M1 | **5,1%** | Повторная покупка в следующий месяц остаётся низкой |
| Сессия → покупка | **26,6%** | Конверсия сессий в покупку в синтетических данных |

### Решения, которые следуют из анализа

1. Запустить CRM-механику второй покупки в первые 30 дней и контролировать **удержание M1**.
2. Разобрать причины возвратов в категории **Plus**, где доля возвратов товаров достигает **12,8%**.
...
| Продуктовая аналитика | Выручка, маржа, отмены, возвраты, повторные покупки, воронка и когортное удержание |
| Клиентская аналитика | RFM-сегментация и оценка вклада сегментов в выручку |
| Статистика | Хи-квадрат для связи доставки и возвратов; z-тест конверсии |
| A/B-тест | Целевая и защитная метрики, доверительный интервал и решение о пилоте |
| Визуализация | 9 воспроизводимых графиков Matplotlib / Seaborn |
| Качество данных | Проверка ключей, связей, хронологии, гранулярности и контрольных сумм |
| Воспроизводимость | Раздельные слои Pandas и SQL, экспорт результатов, манифест и проверочный сценарий |

## Архитектура

```mermaid
flowchart LR
    A["TheLook: 7 исходных таблиц"] --> B["Ноутбук Pandas"]
    A --> C["Набор данных BigQuery"]
    B --> D["order_mart: 1 row = 1 order"]
    C --> E["SQL order_mart"]
    D --> F["KPI · Воронка · Удержание · RFM"]
    E --> F
    F --> G["13 таблиц · 9 графиков"]
    G --> H["Рекомендации бизнесу"]
```
...
### 1. Качество данных и витрина заказов
...
## Как воспроизвести анализ в Pandas
